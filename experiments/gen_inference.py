...
import lib
from lib import (use_bundle, make_env, run_clip, latest_avi, latest_dump,
                 REPO, CLIP_STEPS, FPS, pitch_to_pixel, FRAME_W, FRAME_H)
...
def render_clip(level: str, seed: int, bundle: str, work_dir: Path):
    """Render a 10 s clip into work_dir with the chosen asset bundle."""
    # Asset bundle has to be set BEFORE the env is imported, so we
    # re-use the existing process's gfootball import — both bundles
    # write into a side directory under work_dir.
    use_bundle(bundle)
    env = make_env(level, seed, work_dir, write_video=True, hud=False)
    run_clip(env, steps=CLIP_STEPS, dump_name=f"work_{bundle}")
    env.close()
    return latest_avi(work_dir), latest_dump(work_dir)


def post_occlude_player(in_avi: Path, dump: Path, out_mov: Path,
                        hide_team: str, hide_idx: int, radius: int = 28):
    """Read in_avi, paint a green disc over the chosen player every frame,
    write H.264 mov."""
    cap = cv2.VideoCapture(str(in_avi))
    fps = cap.get(cv2.CAP_PROP_FPS) or FPS
    w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    tmp_avi = out_mov.with_suffix(".tmp.avi")
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    writer = cv2.VideoWriter(str(tmp_avi), fourcc, fps, (w, h))
    GRASS = (44, 110, 32)   # BGR grass-green to paint over the player

    dump_iter = lib.read_dump(dump)
    for idx, step in dump_iter:
        ret, frame = cap.read()
        if not ret:
            break
        ...
        if hide_idx < team.shape[0]:
            ...
            cv2.circle(frame, (px, py), radius, GRASS, -1)
        writer.write(frame)
    cap.release()
    writer.release()
    # Transcode to H.264 fast-start.
    subprocess.run(
        ["ffmpeg", "-y", "-loglevel", "error", "-i", str(tmp_avi),
         "-vf", lib._hud_crop_filter(),
...
def post_occlude_region(in_avi: Path, dump: Path, out_mov: Path,
                        band_y0: int, band_y1: int):
    """Cover a horizontal strip of every frame with black; report how many
    players are inside per frame (we keep the last frame's count as GT)."""
    cap = cv2.VideoCapture(str(in_avi))
    fps = cap.get(cv2.CAP_PROP_FPS) or FPS
    w = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    h = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    for idx, step in lib.read_dump(dump):
        ...
        if not ret:
            break
        cv2.rectangle(frame, (0, band_y0), (w, band_y1), (0, 0, 0), -1)
        obs = step["observation"]
        cnt = 0
        for team_key in ("left_team", "right_team"):
            for p in obs[team_key]:
                _, py = pitch_to_pixel(float(p[0]), float(p[1]))
                if band_y0 <= py <= band_y1:
                    cnt += 1
...
def make_item(level: str, seed: int):
    ...
    avi_ball, dump_ball = render_clip(level, seed, "ball_tiny", work_ball)
    ball_mov = item_dir / "ball_hidden.mov"
    lib.avi_to_mov(avi_ball, ball_mov)

    # 2. player_hidden + region_occluded — both built from a default render.
    work_def = item_dir / "_work_default"
    avi_def, dump_def = render_clip(level, seed, "default", work_def)
    ...
    post_occlude_player(avi_def, dump_def, ph_mov, hide_team, hide_idx)
    ...
    region_counts = post_occlude_region(avi_def, dump_def, ro_mov, band_y0, band_y1)
