...
OUT_NAME = "natural_30_5s_grid"

# ── geometry (matches the project's 1280x480 HUD-cropped render + 32x12 grid) ──
W, H = 1280, 480
CELL = 40
...
def build_grid():
    ...
    for c in range(COLS + 1):
        draw.line([(c * CELL, 0), (c * CELL, H)], fill=line, width=1)
    for r in range(ROWS + 1):
        draw.line([(0, r * CELL), (W, r * CELL)], fill=line, width=1)
    font = _font(11)
    for c in range(COLS):
...
def cell_of(px, py):
    ...
    c = min(max(int(px // CELL), 0), COLS - 1)
    r = min(max(int(py // CELL), 0), ROWS - 1)
