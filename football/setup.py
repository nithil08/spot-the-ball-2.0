...
class CustomBuild(build_ext):
  ...
  def run_unix(self):
    ...
    if use_prebuilt_lib:
      ...
      # Compile the engine. Pin the same interpreter as this build so CMake does
      # not pick another python3 (e.g. Homebrew 3.14) when building a 3.10 wheel.
      # Use bash + subprocess (os.system uses /bin/sh, which can break the script).
      root_dir = os.path.dirname(os.path.abspath(__file__))
      script = os.path.join(root_dir, 'gfootball', 'build_game_engine.sh')
      env = os.environ.copy()
      env['GFOOTBALL_PYTHON'] = sys.executable
      try:
        subprocess.check_call(['/bin/bash', script], cwd=root_dir, env=env)
