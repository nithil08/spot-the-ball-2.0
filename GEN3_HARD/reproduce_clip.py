...
def cmd_pass(tag, clips):
    ...
    for r in recipes(clips):
        ...
        sha = hashlib.sha256((SCENARIOS_DIR / f"g3h_{shape}.py").read_bytes()).hexdigest()
        ok = "match" if sha == r["scenario_sha"] else "DIFFERENT"
        print(f"  [{tag}] {clip}: scenario g3h_{shape} sha {sha[:12]} ({ok})", flush=True)
