...
def cmd_sweep(argv):
    ...
    for shape, seed in todo[:PER_PROCESS]:
        log = G.run_log(f"g3h_{shape}", seed, SWEEP_STEPS)
...
def cmd_ballpix(argv):
    ...
    inv, _ = G.render_window(f"g3h_{p['shape']}", p["seed"], 0, max(ends) + 1,
                             hide_slots=",".join(G.ALL_SLOTS), offset=off)
    out, offscreen = {}, 0
...
def _render(bundle, tag_, shard_i, shard_n):
    ...
    for p in todo[:4]:
        ...
        frames, _ = G.render_window(f"g3h_{p['shape']}", p["seed"], p["start"], p["end"],
                                    hide_slots="",
